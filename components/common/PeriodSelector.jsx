...
export default function PeriodSelector({ value, onChange }) {
  return (
    <div className="flex mb-8 overflow-hidden rounded-full w-fit border border-[var(--border-hair)] p-[3px] gap-0.5"
      style={{ background: 'rgba(250,248,242,0.70)', backdropFilter: 'blur(8px)' }}>
      {PERIODS.map(({ mode, label }) => (
        <button
          key={mode}
          onClick={() => onChange(mode)}
          className={`px-5 py-1.5 rounded-full font-display transition-all duration-300 ease-ink text-[0.72em] tracking-[0.22em]
            ${value === mode
